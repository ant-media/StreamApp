...
require("./external/selfie-segmentation/selfie_segmentation.js");
/**
...
function VideoEffect() {
  ...
  this.createEffectCanvas = function () {
    ...
    this.effectCanvas.width = canvasWidth;
    this.effectCanvas.height = canvasHeight;
    this.effectCanvas.ctx = this.effectCanvas.getContext("2d");
  ...
  this.playing = async function () {
    if (window.videoEffect.rawLocalVideo.readyState === window.videoEffect.rawLocalVideo.HAVE_ENOUGH_DATA && window.videoEffect.selfieSegmentation !== undefined) {
      await window.videoEffect.selfieSegmentation.send({
        image: window.videoEffect.rawLocalVideo
      });
    } else {
  ...
  this.onResults = function (results) {
    window.videoEffect.runPostProcessing(results.image, results.segmentationMask, window.videoEffect.backgroundBlurRange);

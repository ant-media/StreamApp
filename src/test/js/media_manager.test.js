  ...
  describe("setDesktopwithCameraSource", function () {
    ...
    it("should initialize and send frames to the worker", async function () {
      ...
      const mediaManager = new MediaManager({
        userParameters: { mediaConstraints: { video: false, audio: true } },
        webRTCAdaptor: adaptor,
        callback: (info, obj) => adaptor.notifyEventListeners(info, obj),
        callbackError: (error, message) => adaptor.notifyErrorEventListeners(error, message),
        getSender: (streamId, type) => adaptor.getSender(streamId, type),
      });
      ...
      // Mock desktop and camera streams
      const desktopStream = new MediaStream();
      desktopStream.getVideoTracks = () => [mockVideoTrack];
      const cameraStream = new MediaStream();
      cameraStream.getVideoTracks = () => [mockVideoTrack];
      ...
      const workerStub = {
        ...
        terminate: sinon.fake(),
      };
      sinon.stub(window, "Worker").returns(workerStub);

      await mediaManager.setDesktopwithCameraSource(desktopStream, "streamId", null);

      // Assertions for Worker initialization
      ...
      //expect(workerStub.terminate.calledOnce).to.be.true;
    ...
    it("should handle cleanup correctly on stream end", async function () {
      ...
      const onEndedCallback = sinon.fake();
      await mediaManager.setDesktopwithCameraSource(desktopStream, "streamId", onEndedCallback);

      desktopStream.getVideoTracks()[0].onended();

      // Assert cleanup was triggered
      expect(onEndedCallback.calledOnce).to.be.true;
      expect(mediaManager.desktopCameraCanvasDrawerTimer).to.be.null;
